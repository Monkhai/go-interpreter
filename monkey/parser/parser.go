...
func New(l *lexer.Lexer) *Parser {
	...
	p.registerPrefix(token.BANG, p.parsePrefixOperatorExpression)
	p.registerPrefix(token.MINUS, p.parsePrefixOperatorExpression)
...
// returns a boolean of the current token is the type passed to it
func (p *Parser) curTokenIs(t token.TokenType) bool {
	return p.curToken.Type == t
}

// peekTokenIs returns a boolean representing if the next token is the passed type
func (p *Parser) peekTokenIs(t token.TokenType) bool {
	return p.peekToken.Type == t
}

// expectPeek checks if the next token matches and advances if it does
